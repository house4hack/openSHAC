...
boolean index_handler(TinyWebServer& web_server) {
  ...
  if (path.substring(0,6) == "/gate?") {
    ...
    String Submitted_Code_String = path.replace("/gate?", "");
    ...
    String Rolling_Code_String = Submitted_Code_String.replace("&"+LED_Sequence_String, "");
  ...
  else if (path.substring(0,6) == "/resync?") {
    Serial.println("Someone is trying to resync the arduino rolling codes");
    String Submitted_Code_String = path.replace("/resync?", "");
    ...
    Serial.println("Submitted Rolling Code Seed: " + Rolling_Code_String);
    Serial.println("Submitted LED Sequence: " + LED_Sequence_String + " ->" + LED_Sequence_String.substring(0,1));
    // Call that damn stringToUnsignedLong I had to design
    unsigned long Rolling_Code = stringToUnsignedLong(Rolling_Code_String);    
    // Generate the counter Rolling Code Seed supplied by this Arduino
    long arduino_rolling_code_seed = random(0, 2147483647);
    if (valid_button_press() && valid_led_sequence(SubmittedLed1,SubmittedLed2,SubmittedLed3,SubmittedLed4,SubmittedLed5)) {
          web_server << F("<html><body>Resync Success:" + (str)arduino_rolling_code_seed + "</body></html>\n");
    }
